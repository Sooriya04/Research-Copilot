...
## Commit 18 (dev and main) : Implement Python Agentic RAG Retrieval Engine (Phase 3.1)
...
## Commit 19 (dev and main) : Fix Dense Vector Similarity, Sparse GIN Index Query, DB Connection Pool & Dead Code Cleanup

* **Dense Search Vector Math Fix (`services/retrieval_service/main.py`)**:
  * Implemented exact 768-dimensional Cosine Similarity calculation in Python, replacing dummy index-based scoring.
* **FTS GIN Index Optimization (`services/retrieval_service/main.py`)**:
  * Updated sparse search SQL to query `c.search_vector @@ websearch_to_tsquery('english', %s)` directly, leveraging PostgreSQL GIN indexing.
* **DB Connection Health (`services/retrieval_service/main.py`)**:
  * Replaced per-request connection creation with persistent connection pooling and status verification (`SELECT 1`).
* **Dead Code Cleanup (`src/retrieval/`)**:
  * Deleted redundant/bypassed Go retrieval package (`src/retrieval/engine.go`, `rrf.go`, `rrf_test.go`) and unused imports in `src/api/router.go`.
