...
// syncOpenAccessPDF downloads the open access PDF from OpenAlex,
// parses it using the shared PDF extractor microservice, and indexes the paragraphs.
func (c *OpenAlexClient) syncOpenAccessPDF(ctx context.Context, p OpenAlexPaper) {
	if p.PDFURL == nil || *p.PDFURL == "" {
		return
	}

	pdfURL := *p.PDFURL
	paperID := p.PaperID // Default to OpenAlex paper ID
	...
	log.Printf("[INGESTION] [OpenAlex:%s] Downloading PDF from %s", p.PaperID, pdfURL)
	localPath, err := c.Extractor.Download(ctx, paperID, pdfURL)
	if err != nil {
		log.Printf("[INGESTION] [OpenAlex:%s] PDF download failed: %v", p.PaperID, err)
		return
	}

	log.Printf("[INGESTION] [OpenAlex:%s] PDF saved. Extracting text...", p.PaperID)
	extData, err := c.Extractor.Extract(ctx, localPath)
	if err != nil {
		log.Printf("[INGESTION] [OpenAlex:%s] PDF extraction failed: %v", p.PaperID, err)
		return
	}
	...
	if len(p.Abstract) < 10 {
		...
		if extractedAbs != "" {
			...
			log.Printf("[INGESTION] [OpenAlex:%s] Successfully extracted fallback abstract from PDF text: %d chars", p.PaperID, len(p.Abstract))

			// Update openalex_papers table with the newly extracted abstract
			_, err = core.DB.ExecContext(ctx, "UPDATE openalex_papers SET abstract = $1 WHERE paper_id = $2;", p.Abstract, p.PaperID)
			if err != nil {
				log.Printf("[DATABASE] [OpenAlex:%s] Failed to update OpenAlex paper abstract: %v", p.PaperID, err)
			}
		}
	}
	...
	if err == nil && !exists {
		...
		if err != nil {
			log.Printf("[DATABASE] [OpenAlex:%s] Failed to begin arXiv transaction: %v", p.PaperID, err)
			return
		}
		...
		if err != nil {
			arxivTx.Rollback()
			log.Printf("[DATABASE] [OpenAlex:%s] Failed to insert arXiv paper record: %v", p.PaperID, err)
			return
		}
		...
		if err != nil {
			arxivTx.Rollback()
			log.Printf("[DATABASE] [OpenAlex:%s] Failed to prepare paragraphs: %v", p.PaperID, err)
			return
		}
		defer stmt.Close()
		...
		for pIdx, paraText := range paragraphsText {
			if _, err = stmt.ExecContext(ctx, paperID, pIdx, extData.Paragraphs[pIdx].PageNumber, paraText); err != nil {
				arxivTx.Rollback()
				log.Printf("[DATABASE] [OpenAlex:%s] Failed to insert paragraph %d: %v", p.PaperID, pIdx, err)
				return
			}
		}

		if err := arxivTx.Commit(); err != nil {
			log.Printf("[DATABASE] [OpenAlex:%s] Failed to commit arXiv paper transaction: %v", p.PaperID, err)
			return
		}

		log.Printf("[DATABASE] [OpenAlex:%s] Successfully synchronized PDF text to arxiv_papers & paper_paragraphs", p.PaperID)
	}
}
